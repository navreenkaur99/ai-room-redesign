...
function BuyCredits() {
  ...
  const handlePayment = async () => {
    ...
    const options = {
      ...
      handler: async function () {
        // ✅ Add credits in DB
        const addRes = await axios.post("/api/add-credits", {
          email,
          credits: selectedOption.credits,
        });

        // ✅ Update context (so UI instantly updates)
        setUserDetail((prev) => ({
          ...prev,
          credits: addRes.data?.result?.credits ?? (prev?.credits || 0) + selectedOption.credits,
        }));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 p-8">
      <div className="text-center">
        <h2 className="font-bold text-3xl text-indigo-700">Buy More Credits</h2>
        <p className="text-gray-600 mt-2">
          Current Credits: <b>{userDetail?.credits ?? 0}</b>
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-10">
        {creditsOption.map((item, index) => (
          <div
            ...
            className={`relative flex flex-col gap-3 justify-center items-center p-6 rounded-2xl cursor-pointer transition-all duration-300
              bg-white shadow-md hover:shadow-xl hover:scale-105
              ${selectedOption?.credits === item.credits ? "border-2 border-indigo-600 bg-indigo-50" : "border border-gray-200"}`}
          >
            <h2 className="font-bold text-4xl text-indigo-600">{item.credits}</h2>
            <h2 className="font-medium text-gray-700">Credits</h2>
      ...
      {selectedOption && (
        <div className="mt-10 max-w-md mx-auto p-6 bg-white rounded-2xl shadow-lg text-center border border-indigo-200">
          <h3 className="text-2xl font-bold text-indigo-600">{selectedOption.credits} Credits</h3>
          <p className="mt-2 text-gray-600">
            Total: <span className="font-bold text-indigo-700">₹ {selectedOption.amount}</span>
          </p>
